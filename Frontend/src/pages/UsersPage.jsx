...
function UserFormModal({ open, onClose, editUser, onSave }) {
  const [form, setForm] = useState(
    editUser
      ? (() => {
          const [fn, ...ln] = (editUser.name || "").split(" ");
          return {
            firstname: fn,
            lastname: ln.join(" "),
    ...
    <Modal open={open} onClose={onClose} maxWidth={520}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 16 }}>
          <Label>
            No. Telepon{" "}
            <span style={{ color: "var(--muted)" }}>(opsional)</span>
          </Label>
          <Input
            type="tel"
    ...
    <Modal open={open} onClose={onClose} maxWidth={400}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: 14, color: "var(--muted)", marginBottom: 24 }}>
          Data <strong style={{ color: "white" }}>{user?.name}</strong> akan
          dihapus permanen.
        </p>
        <div style={{ display: "flex", gap: 12 }}>
...
export function UsersPage({ showToast }) {
  ...
  // 🔹 letakkan useEffect di sini
  useEffect(() => {
    const checkAuth = async () => {
      try {
        let meRes = await fetch("http://localhost:3001/api/user/me", {
          credentials: "include",
        });
  ...
  const filtered = users.filter((u) => {
    ...
    const matchF =
      filter === "semua" || u.status === filter || u.role === filter;
            ...
            {FILTERS.map((f) => (
              <button
                ...
                style={{
                  ...
                  background:
                    filter === f.id ? "var(--accent-dim)" : "transparent",
